...
- `restart_workflow` on the Expo workflow is effectively unusable: it ALWAYS reports `DIDNT_OPEN_A_PORT` for port 18115 and the supervisor then KILLS the process (curl → HTTP 000 afterward), so the preview shows "crashed". This is platform-side, NOT a code bug: `artifacts/mobile/scripts/dev-start.js` binds a proxy on 0.0.0.0:18115 immediately and `curl localhost:18115/status` returns `{"status":"packager-status:running"}` (HTTP 200) within ~6s — verified — yet Replit's restart detector still times out (it waited 240s). The detector is not honoring the proxy/`/status` trick. The workflow DOES come up fine when started by the system/Run button at session boot (more lenient). Recovery for the user: open the project on replit.com (not the iOS Replit app) and click Run; or connect via Expo Go using the exp:// picard URL while it's running. **This dev-preview quirk does NOT affect App Store publishing** — Expo Launch (Publish button) does a clean production build from source, independent of this dev server.
