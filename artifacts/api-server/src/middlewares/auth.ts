...
/**
 * requireCardNotBlocked — Flex Card debt gate for OUTGOING money only.
 *
 * Must be placed AFTER requireAuth so req.user is populated. Apply ONLY to
 * money-OUT endpoints (spend, transfer, withdraw, buy, tip, boost payment).
 * NEVER apply to login, profile, browsing, selling, incoming money, history,
 * support, or the debt-repayment endpoint itself.
 *
 * Returns 403 with { code: "CARD_BLOCKED", debt } when the card is blocked.
 */
export async function requireCardNotBlocked(req: Request, res: Response, next: NextFunction): Promise<void> {
  const user = req.user;
  if (!user) { res.status(401).json({ error: "Unauthorized" }); return; }

  if ((user as any).flexCardBlocked) {
    const [debt] = await db
      .select()
      .from(flexCardDebtsTable)
      .where(and(eq(flexCardDebtsTable.userId, user.id), eq(flexCardDebtsTable.status, "active")));
    res.status(403).json({
      error: "CARD_BLOCKED",
      code: "CARD_BLOCKED",
      debt: debt
        ...
        : { outstandingUsd: (user as any).flexCardDebtUsd ?? 0 },
    });
    return;
  }
  next();
