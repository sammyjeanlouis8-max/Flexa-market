...
router.post("/listings/:id/boost/initiate", requireAuth, requireNotRestricted, async (req, res): Promise<void> => {
 ...
 */
router.post("/boost/video-only", requireAuth, requireNotRestricted, async (req, res): Promise<void> => {
  const isAdmin = !!(req.user?.isAdmin || req.user?.isSuperAdmin);
 ...
 */
router.post("/listings/:id/boost/stripe-checkout", requireAuth, async (req, res): Promise<void> => {
  const listingId = parseInt(String(req.params.id), 10);
