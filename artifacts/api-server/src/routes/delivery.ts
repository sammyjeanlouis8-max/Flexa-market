...
// ── Haiti department → communes map ──────────────────────────────────────────
// All values lowercase for case-insensitive SQL matching.
const HAITI_DEPARTMENTS: Record<string, string[]> = {
  "Ouest":      ["port-au-prince", "pétion-ville", "petionville", "petion-ville", "delmas", "carrefour", "croix-des-bouquets", "croix des bouquets", "léogâne", "leogane", "kenscoff", "arcahaie", "thomazeau"],
  "Nord":       ["cap-haïtien", "cap-haitien", "cap haïtien", "cap haitien"],
  "Artibonite": ["gonaïves", "gonaives", "saint-marc", "saint marc", "verrettes", "montrouis", "anse-à-galets", "gros-morne"],
  "Centre":     ["hinche", "mirebalais"],
  "Sud":        ["les cayes", "les-cayes"],
  "Grand'Anse": ["jérémie", "jeremie"],
  "Sud-Est":    ["jacmel"],
};

/** All city display names grouped by department (for the driver preference UI). */
export const HAITI_DEPARTMENT_CITIES: Record<string, string[]> = {
  "Ouest":      ["Port-au-Prince", "Pétion-Ville", "Delmas", "Carrefour", "Croix-des-Bouquets", "Léogâne", "Kenscoff", "Arcahaie", "Thomazeau"],
  "Nord":       ["Cap-Haïtien"],
  "Artibonite": ["Gonaïves", "Saint-Marc", "Verrettes", "Montrouis"],
  "Centre":     ["Hinche", "Mirebalais"],
  "Sud":        ["Les Cayes"],
  "Grand'Anse": ["Jérémie"],
  "Sud-Est":    ["Jacmel"],
...
/** Returns lowercase commune names for the given Haiti department. */
function getCommunesForDepartment(department: string): string[] {
...
// Best-effort dual-channel push (web + mobile). Never throws — push is a
// nice-to-have on top of the durable DB notification + SMS already sent.
// IMPORTANT: never include the secret verification code in a push payload.
async function pushBoth(
  ...
  if (!userId) return;
  const payload: any = { title, body, ...(data ? { data } : {}) };
  await Promise.all([
    sendPushToUser(userId, payload).catch(() => {}),
    sendExpoPushToUser(userId, payload).catch(() => {}),
  ]);
}
...
router.get("/delivery/browse", requireAuth, async (req, res): Promise<void> => {
  ...
  if (isAdmin) {
    ...
    const deliveries = rows.map(({ listingImages, ...d }) => {
      const listingImage = Array.isArray(listingImages) && listingImages.length > 0 ? listingImages[0] : null;
      if (d.feeUsd != null) {
        return { ...d, listingImage, distanceFromDriverKm: null, driverEarnings: Math.round(d.feeUsd * DRIVER_COMMISSION_PCT * 100) / 100 };
      }
      if (d.pickupCity && d.deliveryCity && d.country && d.deliveryMethod && d.deliveryMethod !== "self") {
        ...
        return { ...d, listingImage, distanceFromDriverKm: null, feeUsd: calc.feeUsd, feeLocal: calc.feeLocal, distanceKm: calc.distanceKm, driverEarnings: calc.driverEarningsUsd, currency: calc.currency };
      }
      return { ...d, listingImage, distanceFromDriverKm: null, driverEarnings: null };
    });
  ...
  const [driver] = await db
    .select({ status: driversTable.status, latitude: driversTable.latitude, longitude: driversTable.longitude, commune: driversTable.commune, department: (driversTable as any).department })
  ...
  const driverDepartment = (driver as any)?.department ?? null;

  // Department filter: ALL drivers in the same department see the SAME deliveries —
  // no commune-level splits, no GPS radius.  Falls back to commune match if the
  // driver has no department set yet, and shows country-wide if neither is set.
  let locationFilter: ReturnType<typeof sql> | undefined;
  if (driverCta === "approved") {
    const depCommunes = driverDepartment ? getCommunesForDepartment(driverDepartment) : [];
    if (depCommunes.length > 0) {
      locationFilter = sql`LOWER(${deliveriesTable.pickupCity}) = ANY(ARRAY[${sql.join(depCommunes.map(c => sql`${c}::text`), sql`, `)}])`;
    } else if (driverCommune) {
      locationFilter = sql`LOWER(${deliveriesTable.pickupCity}) = LOWER(${driverCommune})`;
    }
    // neither set → no geographic filter, driver sees all country deliveries
  }
...
router.get("/delivery/available", requireAuth, async (req, res): Promise<void> => {
  ...
  const driverLat        = driver.latitude;
  const driverLng        = driver.longitude;
  const driverCommune    = driver.commune;
  const driverDepartment = (driver as any).department ?? null;

  // ── Build WHERE clause ────────────────────────────────────────────────────
  // Department filter: ALL drivers in the same department see the SAME deliveries.
  // Falls back to commune match when no department is set; country-wide if neither.
  let zoneFilter: ReturnType<typeof sql> | undefined;
  const depCommunes = driverDepartment ? getCommunesForDepartment(driverDepartment) : [];
  if (depCommunes.length > 0) {
    zoneFilter = sql`LOWER(${deliveriesTable.pickupCity}) = ANY(ARRAY[${sql.join(depCommunes.map(c => sql`${c}::text`), sql`, `)}])`;
  } else if (driverCommune) {
    zoneFilter = sql`LOWER(${deliveriesTable.pickupCity}) = LOWER(${driverCommune})`;
  }
  ...
  if (driverLat != null && driverLng != null) {
    // Sort by distance from driver GPS — closest pickup first (no radius cap:
    // department filter already scopes the result to the right area)
    deliveries = rows
      .map(d => {
        const coords = d.pickupCity ? lookupCity(d.pickupCity) : null;
        const dist = coords ? haversineKm(driverLat, driverLng, coords[0], coords[1]) : 999;
...
// ─── Delivery state machine (server-side transition guard) ──────────────────
// Canonical, ordered delivery lifecycle. A driver may only advance a delivery
// ONE legal step forward — no skipping ahead and no going backward. Every other
// state change (accept, code verification → delivered, driver-cancel, buyer
// absent, admin/buyer cancel) is owned by its own dedicated, separately-guarded
// endpoint and is intentionally NOT reachable through PATCH /delivery/:id/status.
//
// "arrived_pickup" is the spec's "Arrived at Pickup" step (driver reached the
// merchant). The current driver UI advances driver_assigned → picked_up
// directly, so BOTH paths are allowed until the new driver screens ship.
const DRIVER_STATUS_TRANSITIONS: Record<string, string[]> = {
  driver_assigned: ["arrived_pickup", "picked_up"],
  arrived_pickup:  ["picked_up"],
  picked_up:       ["on_the_way"],
  on_the_way:      ["arrived"],
};

function canDriverAdvance(from: string, to: string): boolean {
  return (DRIVER_STATUS_TRANSITIONS[from] ?? []).includes(to);
}
...
router.post("/delivery/:id/accept", requireAuth, async (req, res): Promise<void> => {
  ...
  await pushBoth(
    delivery.buyerId,
    "Flexa Market 🏍️",
    "Yon chofè aksepte kòmand ou! Louvri app la pou wè kòd sekrè ou.",
...
router.patch("/delivery/:id/status", requireAuth, async (req, res): Promise<void> => {
  ...
  // Reject any transition that skips a step, moves backward, or is not a legal
  // driver advance. Terminal/owned states (delivered, cancelled, buyer_absent)
  // are never reachable here — they each have their own dedicated endpoint.
  if (!canDriverAdvance(delivery.status, status)) {
    res.status(409).json({
      error: "Tranzisyon sa pa pèmèt. Ou dwe swiv etap yo youn apre lòt.",
      currentStatus: delivery.status,
      attempted: status,
      allowedNext: DRIVER_STATUS_TRANSITIONS[delivery.status] ?? [],
    });
    return;
  }
  ...
  // Note: "delivered" status is intentionally excluded from validStatuses above.
  // Delivery confirmation is handled exclusively via POST /delivery/:id/verify-code
  // which requires the driver to be in "arrived" status and enter the buyer's code.

  // Atomic compare-and-set: only advance if the row is still in the exact state
  // we validated and still owned by this driver. Prevents a race with a
  // concurrent cancel/admin/verify-code action from being silently overwritten.
  const [updated] = await db
    .update(deliveriesTable)
    .set(updates)
    .where(and(
      eq(deliveriesTable.id, deliveryId),
      eq(deliveriesTable.status, delivery.status),
  ...
  if (!updated) {
    res.status(409).json({
      error: "Eta livrezon an chanje. Tanpri rafrechi paj la.",
      currentStatus: delivery.status,
    });
    return;
  }

  // Audit log of every legal transition (status, actor, from → to, timestamp)
  req.log.info(
    { deliveryId, driverUserId: userId, from: delivery.status, to: status },
  ...
  // Real-time push for every transition so the buyer/seller tracking screen
...
router.post("/delivery/:id/verify-code", requireAuth, async (req, res): Promise<void> => {
  ...
  // Security guard — driver must have physically arrived before confirming delivery
  // Prevents early/remote confirmation without being at the buyer's location
  if (delivery.status !== "arrived") {
    res.status(400).json({
      error: "Ou dwe mark 'Mwen rive' anvan ou ka konfime livrezon an. Asire ou ke ou nan adrès achtè a.",
      currentStatus: delivery.status,
    });
    return;
  }
  ...
  if (driverEarningsUsd > 0) {
    ...
    // Push driver of wallet credit
    await pushBoth(
      userId,
      "Flexa Market 💰",
      `$${driverEarningsUsd.toFixed(2)} kredite nan kont FM ou — livrezon #FL-${deliveryId} konfime!`,
      { deliveryId, type: "delivery_paid" },
    );
  } else {
...
// GET /api/delivery/driver/zones — return department→commune map for the preference UI
router.get("/delivery/driver/zones", requireAuth, async (_req, res): Promise<void> => {
  res.json({ departments: HAITI_DEPARTMENT_CITIES });
});

// PATCH /api/delivery/driver/location — update GPS + online toggle + zone preference
router.patch("/delivery/driver/location", requireAuth, async (req, res): Promise<void> => {
  const userId = req.userId!;
  const { latitude, longitude, commune, zone, isOnline, department } = req.body;
  ...
  } as any).where(eq(driversTable.userId, userId));
...
router.post("/delivery/:id/driver-cancel", requireAuth, async (req, res): Promise<void> => {
  ...
  if (delivery.buyerId) {
    ...
    await pushBoth(
      delivery.buyerId,
      "Flexa Market",
      "Chofè a anile livrezon an. N ap chèche yon nouvo chofè pou ou — ou p ap peye anyen anplis.",
      { deliveryId, type: "driver_cancelled" },
    );
  }
