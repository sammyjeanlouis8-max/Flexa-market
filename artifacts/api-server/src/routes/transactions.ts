    ...
    const overdueOrders = await db
      ...
      .where(
        and(
          ...
          // ── CRITICAL FIX: never auto-complete an order whose Flexa driver-pool
          // delivery has not actually been delivered. FM-pool deliveries
          // (delivery_method 'motorcycle' / 'car') must release escrow ONLY when a
          // driver completes the delivery (code verification) — never on a timer.
          // Without this gate, an order that NO driver ever accepted (delivery
          // still 'waiting') would auto-release funds to the seller with no
          // delivery ever happening. This is the root cause of orders
          // auto-completing without a driver.
          sql`NOT EXISTS (
            SELECT 1 FROM ${deliveriesTable} d
            WHERE d.transaction_id = ${transactionsTable.id}
              AND d.delivery_method IN ('motorcycle', 'car')
...
router.post("/orders/:id/ship", requireAuth, async (req, res): Promise<void> => {
  ...
  if (isHaiti) {
    ...
    if (useBus) {
      ...
      // listing.city = the city where the item is listed (registered address).
      // sellerUser.location may reflect the seller's phone GPS (e.g. abroad) — use it only as fallback.
      const pickupCity = listing.city ?? sellerUser?.location ?? null;
...
router.post("/orders/:id/confirm-delivery", requireAuth, async (req, res): Promise<void> => {
  ...
  // FM-pool deliveries (Flexa driver: motorcycle/car) release escrow ONLY through
  // the driver's on-delivery secret-code verification — never via a buyer self-
  // confirm. Block this path for any such order whose delivery is not yet
  // 'delivered'. Personal-driver / bus / carrier flows keep their own paths.
  const [fmDelivery] = await db
    .select({ status: deliveriesTable.status, method: deliveriesTable.deliveryMethod })
    .from(deliveriesTable)
  ...
  if (
    fmDelivery &&
    (fmDelivery.method === "motorcycle" || fmDelivery.method === "car") &&
    fmDelivery.status !== "delivered"
  ) {
    res.status(409).json({
      error: "Kòmand sa ap livre pa yon chofè Flexa. Lajan an ap lage otomatikman lè chofè a livre l epi antre kòd sekrè ou. Ou pa bezwen konfime manyèlman.",
      fmDriverDelivery: true,
      deliveryStatus: fmDelivery.status,
    });
    return;
  }
...
router.post("/cart/checkout", requireAuth, requireCardNotBlocked, async (req, res): Promise<void> => {
  const userId = req.userId!;
  const {
    items: rawItems,
  ...
  for (const item of rawItems) {
    ...
    // listing.city is the registered pickup address; location may be the phone's current city abroad
    const sellerCity = (l as any).city ?? sellerUser?.location ?? "Port-au-Prince";
