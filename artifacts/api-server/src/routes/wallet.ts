...
async function getWalletSettings(): Promise<{ rateHtgToUsd: number; rateDopToUsd: number; bonusPct: number; moncashPlatformNumber: string }> {
  ...
  // Use the display rate (rate + spread) so the HTG shown in wallets matches
  // exactly what clients see in the app (the "Afichaj kliyan" value).
...
router.get("/wallet/balance", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    balanceDop: Math.round(spendableUsd * settings.rateDopToUsd),
...
router.post("/wallet/transfer", requireAuth, requireCardNotBlocked, async (req, res): Promise<void> => {
...
router.get("/wallet/history", requireAuth, async (req, res): Promise<void> => {
  ...
  for (const r of allRows) {
    if (r.amountUsd > 0) totalIn  += parseFloat(String(r.amountUsd));
    else                  totalOut += Math.abs(parseFloat(String(r.amountUsd)));
  }
...
router.post("/wallet/topup/card/session", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({ sessionUrl: session.url, paymentRef, sessionId: session.id });
});

// ─── GET /api/wallet/stripe/auto-retry ───────────────────────────────────────
// Called automatically when the wallet page loads (authenticated user).
// Finds every pending Stripe wallet_recharge transaction for this user,
// verifies payment status with Stripe, and credits the wallet if paid.
// Idempotent — safe to call on every page load.
router.get("/wallet/stripe/auto-retry", requireAuth, async (req: any, res): Promise<void> => {
  try {
    const userId = req.userId!;
    ...
    if (!pendingTxs.length) { res.json({ credited: 0, checked: 0 }); return; }

    const stripe = await getStripeClient();
    let credited = 0;

    for (const tx of pendingTxs) {
      // Extract sessionId from note field (format: "Card (Stripe) | session:cs_xxx")
      let sessionId: string | null = tx.note?.match(/session:(cs_[A-Za-z0-9_]+)/)?.[1] ?? null;

      // Fallback: search Stripe by paymentRef metadata (catches old transactions)
      if (!sessionId && tx.paymentRef) {
        try {
          const found = await stripe.checkout.sessions.search({
            query: `metadata["paymentRef"]:"${tx.paymentRef}"`,
            limit: 5,
          });
          ...
          if (sessionId) {
            await db.update(walletTransactionsTable)
              .set({ note: `Card (Stripe) | session:${sessionId}` })
              .where(eq(walletTransactionsTable.id, tx.id));
          }
        } catch { /* Stripe search unavailable — skip */ }
      }
    ...
    res.json({ credited, checked: pendingTxs.length });
  } catch (err) {
    logger.error({ err }, "stripe/auto-retry error");
    res.status(500).json({ credited: 0, checked: 0 });
  }
});
...
router.get("/wallet/referral", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    pendingBalanceUsd: 0,
    totalEarnedUsd: Math.max(0, parseFloat(String(wallet.promoBalance ?? 0)) + parseFloat(String(wallet.unlockedBalance ?? 0))),
  });
});
...
router.post("/wallet/admin/stripe-retry", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (!pendingTxs.length) { res.json({ ok: true, results: [], credited: 0 }); return; }

  const stripe = await getStripeClient();
  const results: { paymentRef: string | null; sessionId: string | null; status: string; amountUsd?: number }[] = [];

  for (const tx of pendingTxs) {
    // Try to extract sessionId from note (format: "Card (Stripe) | session:cs_xxx")
    let sessionId: string | null = tx.note?.match(/session:(cs_[A-Za-z0-9_]+)/)?.[1] ?? null;

    // Fallback: search Stripe by metadata paymentRef (handles old transactions)
    if (!sessionId && tx.paymentRef) {
      try {
        const found = await stripe.checkout.sessions.search({
          query: `metadata["paymentRef"]:"${tx.paymentRef}"`,
        ...
        sessionId = found.data[0]?.id ?? null;
      } catch { /* search not available on account — skip */ }
    }

    if (!sessionId) {
      results.push({ paymentRef: tx.paymentRef, sessionId: null, status: "no_session_id" });
      continue;
    }
    ...
    try {
      ...
      if (session.payment_status !== "paid") {
        results.push({ paymentRef: tx.paymentRef, sessionId, status: session.payment_status });
        continue;
      }
      ...
      logger.error({ err, sessionId, paymentRef: tx.paymentRef }, "Admin stripe-retry: error");
      results.push({ paymentRef: tx.paymentRef, sessionId, status: "error" });
    }
  }

  const credited = results.filter(r => r.status === "credited").length;
  res.json({ ok: true, results, credited });
});
...
router.get("/wallet/admin/user/:id", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  ...
  for (const t of transactions) {
    if (t.amountUsd > 0) totalIn  += parseFloat(String(t.amountUsd));
    else                  totalOut += Math.abs(parseFloat(String(t.amountUsd)));
  }
