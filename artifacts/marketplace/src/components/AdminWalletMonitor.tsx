...
export default function AdminWalletMonitor() {
  ...
  const [creditUser, setCreditUser] = useState<AccountRow | null>(null);
  const [creditAmt, setCreditAmt]   = useState("");
  const [creditNote, setCreditNote] = useState("");
  const [creditLoading, setCreditLoading] = useState(false);
  const [retryingUserId, setRetryingUserId] = useState<number | null>(null);
  ...
  // Pending Stripe recharge recovery
  ...
  const recoverPending = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch("/api/admin/stripe/recover-pending-recharges", {
        method: "POST",
        headers: { Authorization: `Bearer ${tk ?? ""}` },
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error ?? "Echèk");
      toast({
        title: `Rekiperasyon fini ✅`,
        description: `${d.checked} verifye — ${d.recovered} rekipere — ${d.failed} echèk`,
      });
    } catch (e: any) {
      toast({ title: "Echèk rekiperasyon", description: e.message, variant: "destructive" });
    } finally {
      setRecovering(false);
    }
  }, [toast]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            title="Rekipere rechaj Stripe ki pase men kat pa t kredite"
          >
            {recovering
              ? <RefreshCw className="h-3.5 w-3.5 animate-spin" />
              : <DollarSign className="h-3.5 w-3.5" />}
            {recovering ? "Ap rekipere…" : "Rekipere Rechaj"}
          </Button>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-6 px-2 text-[10px] text-green-600 hover:text-green-500"
                        onClick={() => { setCreditUser(a); setCreditAmt(""); setCreditNote(""); }}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-6 px-2 text-[10px] text-orange-500 hover:text-orange-400"
                        title="Retry Stripe pending recharges"
                        disabled={retryingUserId === a.userId}
                        onClick={async () => {
                          setRetryingUserId(a.userId);
                          try {
                            const tk = localStorage.getItem("flexamarket_token");
                            const r = await fetch("/api/wallet/admin/stripe-retry", {
                              method: "POST",
                            ...
                            if (d.ok) {
                              if (d.credited > 0) {
                                toast({ title: `✅ ${d.credited} rechaj Stripe kreditye pou ${a.fullName}` });
                                refetch();
                              } else {
                                toast({ title: "Okenn rechaj Stripe annatant", description: d.results?.map((x: any) => x.status).join(", ") });
                              }
                            } else { toast({ title: "Erè retry", variant: "destructive" }); }
                          } catch { toast({ title: "Erè rezo", variant: "destructive" }); }
                          finally { setRetryingUserId(null); }
                        }}
                      >
                        <RefreshCw className={`h-3 w-3 mr-0.5 ${retryingUserId === a.userId ? "animate-spin" : ""}`} />↺$
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
      ...
      {/* ── Admin Credit Dialog ── */}
      {creditUser && (
        <Dialog open onOpenChange={() => setCreditUser(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle className="text-sm flex items-center gap-2">
                <PlusCircle className="h-4 w-4 text-green-500" />
                Kredit Manèl — {creditUser.fullName}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-3 pt-2">
              <div>
                <p className="text-xs text-muted-foreground mb-1">Balans kounye a</p>
                <p className="text-lg font-black">${creditUser.balanceUsd.toFixed(2)}</p>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold">Montan USD</label>
                <input type="number" step="0.01" min="0.01" placeholder="1.96"
                  value={creditAmt} onChange={e => setCreditAmt(e.target.value)}
                  className="w-full border rounded-lg px-3 py-2 text-sm bg-background" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold">Nòt (opsyonèl)</label>
                <input type="text" placeholder="Stripe recharge manke — 2025-07-25"
              ...
              <div className="flex gap-2 pt-1">
                <Button
                  ...
                  disabled={creditLoading || !creditAmt || parseFloat(creditAmt) <= 0}
                  onClick={async () => {
                    setCreditLoading(true);
                    try {
                      const tk = localStorage.getItem("flexamarket_token");
                      const r = await fetch("/api/wallet/admin/credit", {
                        method: "POST",
                        headers: { "Content-Type": "application/json", ...(tk ? { Authorization: `Bearer ${tk}` } : {}) },
                        body: JSON.stringify({ userId: creditUser.userId, amountUsd: parseFloat(creditAmt), note: creditNote || "Admin credit manèl" }),
                      });
                      const d = await r.json();
                      if (d.ok) {
                        toast({ title: `✅ $${parseFloat(creditAmt).toFixed(2)} kreditye sou ${creditUser.fullName}` });
                        setCreditUser(null); refetch();
                      } else { toast({ title: "Erè", description: d.error ?? "Echèk", variant: "destructive" }); }
                    } catch { toast({ title: "Erè rezo", variant: "destructive" }); }
                    finally { setCreditLoading(false); }
                  }}
                >
                  {creditLoading ? "Ap kredit…" : `Kredit $${parseFloat(creditAmt || "0").toFixed(2)}`}
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setCreditUser(null)} className="text-xs">Anile</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
