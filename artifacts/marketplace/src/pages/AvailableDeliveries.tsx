...
function DeliveryBrowseCard({ delivery, driverCta, onAcceptClick, onSkip, driverRating, isAccepting, onNavigate }: {
  ...
  const paymentLabel = (pm: string | null | undefined) => {
    if (!pm) return "—";
    const map: Record<string, string> = { wallet: "Wallet FM", card: "Kat Kredi", moncash: "MonCash", usdt: "USDT", sepa: "SEPA", apple_pay: "Apple Pay" };
    return map[pm] ?? pm;
  };
    ...
    <div className={`bg-card border rounded-3xl overflow-hidden shadow-sm transition-all duration-200 ${
      ...
      {hasTip && (
        <div className="bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-1.5 flex items-center gap-2">
          ...
          <span className="text-white text-[11px] font-black uppercase tracking-wide">Livrezon Prioritè</span>
      ...
      <div className="p-4 space-y-4">
        ...
        {/* ── Row 1: Vehicle/tier info + Price card ── */}
        <div className="flex items-start gap-3">
          {/* Left: icon + type + tier + time */}
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isMoto ? "bg-orange-100 dark:bg-orange-900/30" : "bg-blue-100 dark:bg-blue-900/30"}`}>
                {isMoto ? <Bike className="h-5 w-5 text-orange-600" /> : <Car className="h-5 w-5 text-blue-600" />}
              </div>
              <div className="flex flex-col">
                <span className="text-xs font-bold text-foreground">
                  {isMoto ? t("availableDeliveries.motoLabel") : t("availableDeliveries.carLabel")}
                </span>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md w-fit ${speedCfg.bg} ${speedCfg.color}`}>
                  ⚡ {speedCfg.label}
                </span>
              </div>
            </div>
            <p className="text-3xl font-black text-foreground leading-none">{speedCfg.time}</p>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              {delivery.createdAt ? `${timeAgo(delivery.createdAt, t("availableDeliveries.timeNow"))} de sa` : ""}
            </p>
          </div>

          {/* Right: Earnings card — prominent green */}
          <div className={`rounded-2xl px-3 py-3 text-center shrink-0 min-w-[100px] ${
            displayEarnings > 0
              ? "bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-200 dark:shadow-emerald-900/40"
              : "bg-muted border border-dashed border-border text-foreground"
          }`}>
            <p className={`text-[9px] font-black uppercase tracking-wider ${displayEarnings > 0 ? "text-emerald-100" : "text-muted-foreground"}`}>
              💰 OU AP TOUCHE
            </p>
            ...
            {localEarnings != null && localEarnings > 0 && localCurrency && (
              <p className={`text-[10px] font-bold mt-0.5 ${displayEarnings > 0 ? "text-emerald-100" : "text-muted-foreground"}`}>
                ≈ {localEarnings.toLocaleString()} {localCurrency}
              </p>
            )}
            {hasTip && (
              <p className="text-[9px] bg-white/20 rounded-full px-1.5 py-0.5 mt-1 font-bold">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            },
            {
              icon: "⭐",
              label: "Evalyasyon",
              value: driverRating > 0 ? `${driverRating.toFixed(1)} / 5` : "Nouvo",
            },
            {
              icon: "🛍️",
              label: "Kantite atik",
        ...
        {/* ── Product info (from seller's post) ── */}
        {delivery.listingTitle && (
          <div className="flex items-center gap-3 bg-muted/50 rounded-2xl px-3 py-2.5">
            {delivery.listingImage ? (
              ...
              <div className="h-12 w-12 rounded-xl bg-muted flex items-center justify-center shrink-0 border border-border">
                <Package className="h-5 w-5 text-muted-foreground" />
              </div>
            )}
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-0.5">📦 Pwodwi pou livrezon</p>
        ...
        <div className="flex items-stretch gap-3">
          ...
          <div className="flex-1 space-y-3 min-w-0">
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">COLLECTE</p>
            ...
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">LIVRAISON</p>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-black uppercase tracking-wider text-emerald-800 dark:text-emerald-300">
                  {expanded ? "FÈMEN DETAY YO" : "OUVRI POU WÈ DETAY KONPLÈ"}
                </p>
                {!expanded && (
                  <p className="text-[10px] text-emerald-700/70 dark:text-emerald-400/70">
                    Adrès, nòt, nimewo tel, ak plis detay
                  </p>
                )}
              </div>
              {expanded ? (
              ...
              <div className="bg-card px-4 py-3 space-y-3">
                ...
                {delivery.buyerPhone && (
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">📱 Nimewo Telefòn</p>
                ...
                {delivery.paymentMethod && (
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">💵 Metòd Peman</p>
                    <p className="text-sm font-semibold text-foreground">{paymentLabel(delivery.paymentMethod)}</p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  {([
                    { label: "🏷️ ID Livrezon",      value: `FM-${delivery.id}` },
                    { label: "📦 Nimewo Order",     value: delivery.transactionIdNum != null ? `ORD-${delivery.transactionIdNum}` : null },
                    { label: "💳 ID Tranzaksyon",   value: delivery.transactionIdNum != null ? `TRX-${delivery.transactionIdNum}` : null },
          ...
          <div className="space-y-2 pt-1">
            ...
            >
              {isAccepting
                ...
                : <><CheckCircle className="h-4 w-4 mr-2" /> ✓ Aksepte livrezon</>
              }
            </Button>
            <Button
            ...
            >
              <Flag className="h-3.5 w-3.5" /> Rapòte yon pwoblèm
            </button>
          </div>
        ) : driverCta === "pending" ? (
          <Button
...
function ActiveDeliveryCard({ delivery, onUpdateStatus, onDriverCancel, updating, token }: {
  ...
  const STATUS_CONFIG: Record<string, { label: string; color: string; step: number }> = {
    ...
    picked_up:       { label: t("availableDeliveries.statusPickedUp"),   color: "bg-purple-500",  step: 2 },
    on_the_way:      { label: t("availableDeliveries.statusOnWay"),      color: "bg-primary",     step: 3 },
    arrived:         { label: t("availableDeliveries.statusArrived"),    color: "bg-emerald-500", step: 4 },
    delivered:       { label: t("availableDeliveries.statusDelivered"),  color: "bg-green-600",   step: 5 },
    failed_pickup:   { label: "Machann Pa Prezan",                       color: "bg-red-500",     step: 1 },
    buyer_absent:    { label: "Achtè Pa Disponib ⏳",                    color: "bg-amber-500",   step: 4 },
    returning:       { label: "Retou an Kour 🔄",                        color: "bg-indigo-500",  step: 4 },
    seller_closed:   { label: "Machann Fèmen 🔒",                        color: "bg-red-700",     step: 4 },
    returned:        { label: "Retou Konfime ✓",                         color: "bg-slate-500",   step: 5 },
  };
  ...
  const nextLabel: Record<string, string> = {
    driver_assigned: t("availableDeliveries.nextPickedUp"),
  ...
  const isBeforeArrived = ["driver_assigned", "picked_up", "on_the_way"].includes(delivery.status);
        ...
        {isArrived && showCodeEntry ? (
          ...
          <div className="space-y-3">
            ...
            >
              {updating
                ? <Loader2 className="h-5 w-5 animate-spin" />
                : delivery.status === "on_the_way" ? "Mwen rive kote a" : nextLabel[delivery.status]}
            </Button>
