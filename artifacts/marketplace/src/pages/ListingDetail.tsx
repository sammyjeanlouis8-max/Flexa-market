...
export default function ListingDetail() {
  ...
  const [payStep, setPayStep] = useState<"promo" | "address" | "delivery" | "select" | "usdt" | "moncash" | "natcash" | "wallet" | "friend">("promo");
  const [walletPayLoading, setWalletPayLoading] = useState(false);
  const [friendQuery, setFriendQuery] = useState("");
  const [friendSearching, setFriendSearching] = useState(false);
  const [friendResults, setFriendResults] = useState<Array<{id: number; name: string; avatar: string|null; country: string|null}>>([]);
  const [friendRecipient, setFriendRecipient] = useState<{id: number; name: string; avatar: string|null} | null>(null);
  ...
  // Auto-fill the shipping form from the buyer's registered profile.
  // Only fills fields the buyer hasn't already typed (or restored from a saved
  ...
  const searchFriendUsers = async () => {
    ...
    try {
      const res = await fetch(`/api/wallet/p2p/search?q=${encodeURIComponent(friendQuery)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) setFriendResults((await res.json()).users ?? []);
    } finally { setFriendSearching(false); }
  };

  const handleFriendPayRequest = async () => {
    if (!friendRecipient || !listing) return;
    setFriendSending(true);
    try {
      await fetch("/api/friend-pay-request", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ listingId: listing.id, toUserId: friendRecipient.id, amount: effectiveListingPriceUsd }),
      });
    } catch {}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="divide-y divide-border pb-8">
        ...
        <div className="px-4 py-4 space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950/30 rounded-lg px-3 py-2">
            <CheckCircle2 className="h-3.5 w-3.5 flex-shrink-0" />
            <span>{t("tr.freeShippingOn", { count: buyQty })}</span>
          </div>
        </div>
        ...
        {/* ── DESCRIPTION ── */}
        <div className="px-4 py-4 space-y-2.5">
          <h2 className="text-xs font-black uppercase tracking-widest text-muted-foreground">{t("listing.description")}</h2>
        ...
        {country && (
          <div className="px-4 pb-4">
            <div className="rounded-2xl border border-border bg-card p-4 space-y-2.5">
              <h2 className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-1">{t("tr.deliveryReturns")}</h2>
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-green-50 dark:bg-green-950/20 border border-green-200/60 dark:border-green-800/40">
                ...
                <div>
                  <p className="text-sm font-bold text-green-800 dark:text-green-300">
                    {t("tr.returnDays", { count: getReturnDays(country) })}
                  </p>
              ...
              {isHaitiListingCountry ? (
                <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-800/40">
                  ...
                  <div>
                    <p className="text-sm font-bold text-blue-800 dark:text-blue-300">{t("tr.deliveryDriver")}</p>
                    <p className="text-xs text-blue-700/80 dark:text-blue-400/80 mt-0.5">{t("tr.deliveryDriverDesc")}</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-800/40">
                  <span className="text-xl mt-0.5">📦</span>
                  <div>
                    <p className="text-sm font-bold text-blue-800 dark:text-blue-300">{t("tr.localDelivery")}</p>
                    <p className="text-xs text-blue-700/80 dark:text-blue-400/80 mt-0.5">{t("tr.localDeliveryDesc")}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-violet-50 dark:bg-violet-950/20 border border-violet-200/60 dark:border-violet-800/40">
                <span className="text-xl mt-0.5">🔒</span>
                <div>
                  <p className="text-sm font-bold text-violet-800 dark:text-violet-300">{t("tr.escrow")}</p>
                  <p className="text-xs text-violet-700/80 dark:text-violet-400/80 mt-0.5">{t("tr.escrowDesc")}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <>
              ...
              {payStep === "promo" && (
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-amber-200 dark:border-amber-700/50 bg-amber-50/50 dark:bg-amber-950/20 px-2.5 py-2 flex items-center gap-2">
                    ...
                    <p className="text-[10px] text-amber-700 dark:text-amber-400 flex-1 leading-snug">{t("tr.shippingFeePre")} <strong>{t("tr.shippingFeeBold")}</strong> {t("tr.shippingFeePost")}</p>
                  </div>
              ...
              {payStep === "select" && (
                <>
                  ...
                  <div className="space-y-3 mt-3">
                    ...
                    {/* ── Zanmi Peye ── */}
                    {user && (
                      <button onClick={() => { setFriendQuery(""); setFriendResults([]); setFriendRequestSent(false); setPayStep("friend"); }}
                        className="w-full flex items-center gap-4 p-4 rounded-xl border-2 border-border hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-all text-left"
                        data-testid="button-pay-friend">
                        <div className="w-10 h-10 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center flex-shrink-0">
                          <Users className="h-5 w-5 text-emerald-600" />
                        </div>
                        <div>
                          <p className="font-semibold">{t("friendPay.title")}</p>
                          <p className="text-xs text-muted-foreground">{t("friendPay.subtitle")}</p>
                        </div>
                      </button>
                    )}
                  </div>
              ...
              {payStep === "friend" && (
                <>
                  <DialogHeader>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setPayStep("select")} className="p-1 rounded hover:bg-muted">
                        <ChevronLeft className="h-4 w-4" />
                      </button>
                      <DialogTitle>{t("friendPay.title")}</DialogTitle>
                    </div>
                  </DialogHeader>
                  <div className="space-y-4 mt-2">
                    {!friendRequestSent ? (
                      <>
                        <p className="text-sm text-muted-foreground">{t("friendPay.subtitle")}</p>
                        <div className="flex gap-2">
                          <div className="relative flex-1">
                            ...
                            <Input
                              ...
                              onKeyDown={e => e.key === "Enter" && searchFriendUsers()}
                              placeholder={t("friendPay.searchPlaceholder")}
                              className="pl-9 rounded-xl"
                            />
                          </div>
                          <Button onClick={searchFriendUsers} disabled={friendSearching} className="rounded-xl">
                            {friendSearching ? <div className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : t("walletTransfer.search")}
                          </Button>
                        </div>
                          ...
                          <div className="space-y-2 max-h-64 overflow-y-auto">
                            {friendResults.map(u => (
                              <button
                                key={u.id}
                                type="button"
                                onClick={() => { setFriendRecipient(u); setFriendConfirmOpen(true); }}
                      ...
                      <div className="text-center py-6">
                        ...
                        <h3 className="font-black text-lg mb-1">{t("friendPay.requestSent")}</h3>
                        <p className="text-sm text-muted-foreground">{friendRecipient?.name}</p>
                        <Button variant="outline" className="mt-4" onClick={() => setBuyNowOpen(false)}>
                          {t("buttons.close", { defaultValue: "Fèmen" })}
                        </Button>
                      </div>
                    )}
                  </div>
      ...
      {/* ── Friend Pay Confirmation Dialog ── */}
      <Dialog open={friendConfirmOpen} onOpenChange={v => { if (!friendSending) setFriendConfirmOpen(v); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-amber-500" />
              {t("friendPay.warningTitle")}
            </DialogTitle>
          </DialogHeader>
          {friendRecipient && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-2xl">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={friendRecipient.avatar ?? undefined} />
                  <AvatarFallback className="bg-primary/10 text-primary font-bold text-lg">{friendRecipient.name?.[0]}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-bold">{friendRecipient.name}</p>
                </div>
              </div>
              <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
                <p className="text-sm text-amber-800 dark:text-amber-300">⚠️ {t("friendPay.warningText")}</p>
              </div>
            </div>
          )}
          <DialogFooter className="flex gap-2 flex-col sm:flex-row pt-2">
            <Button variant="outline" className="flex-1" onClick={() => setFriendConfirmOpen(false)} disabled={friendSending}>
              {t("friendPay.cancelBtn")}
            </Button>
            <Button className="flex-1" onClick={handleFriendPayRequest} disabled={friendSending}>
              {friendSending ? <div className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : (
                <><Send className="h-4 w-4 mr-1.5" /> {t("friendPay.confirmBtn")}</>
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
