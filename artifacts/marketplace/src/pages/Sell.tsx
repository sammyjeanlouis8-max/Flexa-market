...
function ReqDot({ filled }: { filled: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
        "inline-block h-2 w-2 rounded-full ml-1.5 align-middle shrink-0 transition-colors",
...
export default function Sell() {
  ...
  const selectPayoutMethod = useCallback(async (method: "fm_wallet" | "stripe") => {
    ...
    try {
      ...
      toast({ title: t("common.error"), description: t("tr.paymentSaveErrorDesc"), variant: "destructive" });
    } finally {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          ...
          <FormField control={form.control} name="title" render={({ field }) => (
            <FormItem>
              <FormLabel>{t("sell.listingTitle")}<ReqDot filled={(field.value ?? "").trim().length >= 3} /></FormLabel>
          ...
          <FormField control={form.control} name="description" render={({ field }) => (
            <FormItem>
              <FormLabel>{t("sell.description")}<ReqDot filled={(field.value ?? "").trim().length >= 10} /></FormLabel>
          ...
          <FormField control={form.control} name="price" render={({ field }) => (
            <FormItem>
              <div className="flex items-center justify-between mb-1.5">
                <FormLabel className="mb-0">{t("sell.price")}<ReqDot filled={Number(field.value) > 0} /></FormLabel>
          ...
          <FormField control={form.control} name="condition" render={({ field }) => (
            <FormItem>
              <FormLabel>{t("sell.condition")}<ReqDot filled={!!field.value} /></FormLabel>
              <FormControl>
                <select
                  value={field.value}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="location" render={({ field }) => (
              <FormItem>
                <FormLabel>{t("sell.location")}<ReqDot filled={(field.value ?? "").trim().length >= 2} /></FormLabel>
