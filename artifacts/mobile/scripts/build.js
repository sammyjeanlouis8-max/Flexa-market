...
async function downloadFile(url, outputPath) {
  ...
  try {
    ...
    if (!response.ok) {
      let errorBody = "";
      try { errorBody = await response.text(); } catch { errorBody = "(could not read body)"; }
      console.error(`[Metro Build Error] HTTP ${response.status} for ${url}`);
      console.error(`[Metro Build Error] Response body:\n${errorBody.slice(0, 5000)}`);
